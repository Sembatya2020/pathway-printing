  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-50 h-[80px] transition-all duration-300 ${scrolled ? 'bg-charles-darker/95 backdrop-blur-md shadow-lg' : 'bg-charles-darker'}`}>
      <div className="mx-auto flex h-full max-w-7xl items-center justify-between px-6 md:px-10">
        <Link href="/" className="relative h-[65px] md:h-[80px] w-[260px] md:w-[340px] transition-transform hover:opacity-80 flex items-center overflow-hidden">
          <Image
            src="/images/Finished designs/Logo.png"
            alt="Pathway Printing & Graphics"
            fill
            unoptimized
            className="object-contain object-left scale-[1.6] md:scale-[1.9] origin-left"
            priority
          />
        </Link>
      ...
      <a
        href="https://wa.me/256755960836"
        className="fixed bottom-6 right-6 z-50 rounded-full bg-green-500 hover:bg-green-600 px-4 py-3 text-sm font-bold text-white shadow-xl transition-all hover:scale-105 md:text-base flex items-center"
      >
